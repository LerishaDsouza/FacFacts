...
if(isset($_POST['add'])){
	$q = $_POST['q'];
                  ...
                  $i= $_POST['i'];
                  $j= $_POST['j'];
	$query = mysqli_query($db,"INSERT INTO annauniversity values('$q','$m','$n','$h','$i','$j')"); 
                  $queryr= mysqli_query($db,"SELECT * from annauniversity"); 
...
if(isset($_POST['delete'])){
	$q = $_POST['q'];
                  ...
                  $i= $_POST['i'];
                  $j= $_POST['j'];
	$query = mysqli_query($db,"DELETE from annauniversity where Name LIKE '%$q%'"); 
                  $queryr= mysqli_query($db,"SELECT * from annauniversity"); 
//Replace table_name with your table name and `thing_to_search` with the column you want to search
	$count = mysqli_num_rows($queryr);
	if($count == "0"){
...
if(isset($_POST['edit'])){
	$q = $_POST['q'];
                  ...
                  $i= $_POST['i'];
                  $j= $_POST['j'];
	$query = mysqli_query($db,"UPDATE annauniversity SET Collegename='$m' where Name='$q'"); 
                  $queryr= mysqli_query($db,"SELECT * from annauniversity"); 
//Replace table_name with your table name and `thing_to_search` with the column you want to search
	$count = mysqli_num_rows($queryr);
	if($count == "0"){
