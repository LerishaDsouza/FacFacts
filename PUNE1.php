...
<h1 align="center">PUNE UNIVERSITY</h1>
<div class="container">
...
if(isset($_POST['add'])){
	$q = $_POST['q'];
                  ...
                  $i= $_POST['i'];
                  $j= $_POST['j'];
	$query = mysqli_query($db,"INSERT INTO puneuniversity values('$q','$m','$n','$h','$i','$j')"); 
                  $queryr= mysqli_query($db,"SELECT * from puneuniversity"); 
...
if(isset($_POST['delete'])){
	$q = $_POST['q'];
                  ...
                  $i= $_POST['i'];
                  $j= $_POST['j'];
	$query = mysqli_query($db,"DELETE from puneuniversity where Name LIKE '%$q%'"); 
                  $queryr= mysqli_query($db,"SELECT * from puneuniversity"); 
//Replace table_name with your table name and `thing_to_search` with the column you want to search
	$count = mysqli_num_rows($queryr);
	if($count == "0"){
...
if(isset($_POST['edit'])){
	$q = $_POST['q'];
                  ...
                  $i= $_POST['i'];
                  $j= $_POST['j'];
	$query = mysqli_query($db,"UPDATE puneuniversity SET Collegename='$m' where Name='$q'"); 
                  $queryr= mysqli_query($db,"SELECT * from puneuniversity"); 
//Replace table_name with your table name and `thing_to_search` with the column you want to search
	$count = mysqli_num_rows($queryr);
	if($count == "0"){
